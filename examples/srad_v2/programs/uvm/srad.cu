...
runTest( int argc, char** argv) 
{
    ...
    CUDA_CALL_SAFE(cudaMallocManaged(&J, size_I * sizeof(float)));
    filepath = (char *)malloc(sizeof(char) * (strlen(folder) + 128));


	//Allocate device memory
    CUDA_CALL_SAFE(cudaMallocManaged((void**)& C_cuda, sizeof(float)* size_I));
	CUDA_CALL_SAFE(cudaMallocManaged((void**)& E_C, sizeof(float)* size_I));
	CUDA_CALL_SAFE(cudaMallocManaged((void**)& W_C, sizeof(float)* size_I));
	CUDA_CALL_SAFE(cudaMallocManaged((void**)& S_C, sizeof(float)* size_I));
	CUDA_CALL_SAFE(cudaMallocManaged((void**)& N_C, sizeof(float)* size_I));

    J_cuda = J;

    gettimeofday(&tv_start, NULL);
    sprintf(filepath, "%s/J.uvm.mem", folder);
