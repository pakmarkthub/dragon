...
void run(int argc, char **argv)
{
    ...
    if ((fd_t = open(tfile, O_LARGEFILE | O_RDWR)) < 0)
    {
        fprintf(stderr, "Cannot open file %s\n", tfile);
        exit(EXIT_FAILURE);
    }
    if ((MatrixTemp[0] = (float *)mmap(NULL, sizeof(float) * size, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_NORESERVE, fd_t, 0)) == MAP_FAILED)
    {
        fprintf(stderr, "Cannot mmap %s\n", tfile);
        exit(EXIT_FAILURE);
    }
    CUDA_CALL_SAFE(cudaHostRegister(MatrixTemp[0], sizeof(float) * size, cudaHostRegisterDefault));

    if ((fd_p = open(pfile, O_LARGEFILE | O_RDWR)) < 0)
    {
        fprintf(stderr, "Cannot open file %s\n", pfile);
        exit(EXIT_FAILURE);
    }
    if ((MatrixPower = (float *)mmap(NULL, sizeof(float) * size, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_NORESERVE, fd_p, 0)) == MAP_FAILED)
    {
        fprintf(stderr, "Cannot mmap %s\n", pfile);
        exit(EXIT_FAILURE);
    }
    CUDA_CALL_SAFE(cudaHostRegister(MatrixPower, sizeof(float) * size, cudaHostRegisterDefault));

    if ((fd_o = open(ofile, O_LARGEFILE | O_RDWR | O_CREAT)) < 0)
    {
        fprintf(stderr, "Cannot open file %s\n", ofile);
        exit(EXIT_FAILURE);
    }
    if (ftruncate(fd_o, sizeof(float) * size) != 0)
    {
        fprintf(stderr, "Cannot truncate file %s\n", fd_o);
        exit(EXIT_FAILURE);
    }
    if ((MatrixTemp[1] = (float *)mmap(NULL, sizeof(float) * size, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_NORESERVE, fd_o, 0)) == MAP_FAILED)
    {
        fprintf(stderr, "Cannot mmap %s\n", ofile);
        exit(EXIT_FAILURE);
    }
    CUDA_CALL_SAFE(cudaHostRegister(MatrixTemp[1], sizeof(float) * size, cudaHostRegisterDefault));
    ...
    gettimeofday(&tv_start, NULL);
    CUDA_CALL_SAFE(cudaHostUnregister(MatrixTemp[0]));
    if (msync(MatrixTemp[0], sizeof(float) * size, MS_SYNC) != 0)
    {
        fprintf(stderr, "Cannot msync MatrixTemp[0]\n");
        perror("msync");
        exit(EXIT_FAILURE);
    }
    if (munmap(MatrixTemp[0], sizeof(float) * size) != 0)
    {
        fprintf(stderr, "Cannot munmap MatrixTemp[0]\n");
        perror("munmap");
        exit(EXIT_FAILURE);
    }
    fsync(fd_t);
    ...
    CUDA_CALL_SAFE(cudaHostUnregister(MatrixPower));
    if (msync(MatrixPower, sizeof(float) * size, MS_SYNC) != 0)
    {
        fprintf(stderr, "Cannot msync MatrixPower\n");
        perror("msync");
        exit(EXIT_FAILURE);
    }
    if (munmap(MatrixPower, sizeof(float) * size) != 0)
    {
        fprintf(stderr, "Cannot munmap MatrixPower\n");
        perror("munmap");
        exit(EXIT_FAILURE);
    }
    fsync(fd_p);
    ...
    CUDA_CALL_SAFE(cudaHostUnregister(MatrixTemp[1]));
    if (msync(MatrixTemp[1], sizeof(float) * size, MS_SYNC) != 0)
    {
        fprintf(stderr, "Cannot msync MatrixTemp[1]\n");
        perror("msync");
        exit(EXIT_FAILURE);
    }
    if (munmap(MatrixTemp[1], sizeof(float) * size) != 0)
    {
        fprintf(stderr, "Cannot munmap MatrixTemp[1]\n");
        perror("munmap");
        exit(EXIT_FAILURE);
    }
    fsync(fd_o);
