...
void run(int argc, char **argv)
{
    ...
    if (dragon_map(tfile, sizeof(float) * size, D_F_READ | D_F_WRITE | D_F_USEHOSTBUF, (void **)&MatrixTemp[0]) != D_OK)
    {
        fprintf(stderr, "Cannot dragon_map %s\n", tfile);
        exit(EXIT_FAILURE);
    }

    if (dragon_map(pfile, sizeof(float) * size, D_F_READ | D_F_USEHOSTBUF, (void **)&MatrixPower) != D_OK)
    {
        fprintf(stderr, "Cannot dragon_map %s\n", pfile);
        exit(EXIT_FAILURE);
    }

    if (dragon_map(ofile, sizeof(float) * size, D_F_WRITE | D_F_CREATE | D_F_USEHOSTBUF, (void **)&MatrixTemp[1]) != D_OK)
    {
        fprintf(stderr, "Cannot dragon_map %s\n", ofile);
        exit(EXIT_FAILURE);
    }
    ...
    gettimeofday(&tv_start, NULL);
    if (dragon_unmap(MatrixPower) != D_OK)
    {
        fprintf(stderr, "Cannot dragon_unmap MatrixPower\n");
        exit(EXIT_FAILURE);
    }

    if (dragon_unmap(MatrixTemp[0]) != D_OK)
    {
        fprintf(stderr, "Cannot dragon_unmap MatrixTemp[0]\n");
        exit(EXIT_FAILURE);
    }

    if (dragon_unmap(MatrixTemp[1]) != D_OK)
    {
        fprintf(stderr, "Cannot dragon_unmap MatrixTemp[1]\n");
        exit(EXIT_FAILURE);
    }
    gettimeofday(&tv_end, NULL);
