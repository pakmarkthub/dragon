...
void init(int argc, char** argv)
{
    ...
    if ((fd_data = open(filepath, O_LARGEFILE | O_RDWR)) < 0)
    {
        fprintf(stderr, "Cannot open file %s\n", filepath);
        exit(EXIT_FAILURE);
    }
    if ((data = (int *)mmap(NULL, sizeof(int) * rows * cols, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_NORESERVE, fd_data, 0)) == MAP_FAILED)
    {
        fprintf(stderr, "Cannot mmap %s\n", filepath);
        exit(EXIT_FAILURE);
    }
    CUDA_CALL_SAFE(cudaHostRegister(data, sizeof(int) * rows * cols, cudaHostRegisterDefault));

    sprintf(filepath, "%s/result.hostreg.mem", folder);
    if ((fd_result = open(filepath, O_LARGEFILE | O_RDWR | O_CREAT)) < 0)
    {
        fprintf(stderr, "Cannot open file %s\n", filepath);
        exit(EXIT_FAILURE);
    }
    if (ftruncate(fd_result, sizeof(int) * cols) != 0)
    {
        fprintf(stderr, "Cannot truncate file %s\n", filepath);
        exit(EXIT_FAILURE);
    }
    if ((result = (int *)mmap(NULL, sizeof(int) * cols, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_NORESERVE, fd_result, 0)) == MAP_FAILED)
    {
        fprintf(stderr, "Cannot mmap %s\n", filepath);
        exit(EXIT_FAILURE);
    }
    CUDA_CALL_SAFE(cudaHostRegister(result, sizeof(int) * cols, cudaHostRegisterDefault));
...
void run(int argc, char** argv)
{
    ...
    gettimeofday(&tv_start, NULL);
    CUDA_CALL_SAFE(cudaHostUnregister(data));
    if (munmap(data, sizeof(int) * rows * cols) != 0)
    {
        fprintf(stderr, "Cannot munmap data\n");
        perror("munmap");
        exit(EXIT_FAILURE);
    }
    close(fd_data);

    CUDA_CALL_SAFE(cudaHostUnregister(result));
    if (msync(result, sizeof(int) * cols, MS_SYNC) != 0)
    {
        fprintf(stderr, "Cannot msync result\n");
        perror("msync");
        exit(EXIT_FAILURE);
    }
    if (munmap(result, sizeof(int) * cols) != 0)
    {
        fprintf(stderr, "Cannot munmap result\n");
        perror("munmap");
        exit(EXIT_FAILURE);
    }
    fsync(fd_result);
