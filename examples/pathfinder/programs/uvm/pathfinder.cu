...
void init(int argc, char** argv)
{
    ...
    CUDA_CALL_SAFE(cudaMallocManaged(&data, sizeof(int) * rows * cols));
...
void run(int argc, char** argv)
{
    ...
    CUDA_CALL_SAFE(cudaMallocManaged((void**)&gpuResult[0], sizeof(int)*cols));
    CUDA_CALL_SAFE(cudaMallocManaged((void**)&gpuResult[1], sizeof(int)*cols));
    ...
    gettimeofday(&tv_start, NULL);
    sprintf(filepath, "%s/result.uvm.mem", folder);
